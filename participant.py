...
async def add_participant(call: types.CallbackQuery):
    ...
    try:
        ...
        logger.warning(f'User "{call.from_user.id} - {call.from_user.username}" except "MessageCantBeDeleted"')
    ...
    logger.info(f'User "{call.from_user.id} - {call.from_user.username}" push "add_participant"')
...
async def add_participants(call: types.CallbackQuery):
    ...
    logger.info(f'User "{call.from_user.id} - {call.from_user.username}" push "add_participants"')
    await call.answer()
...
async def add_participants_db(msg: types.Message, state: FSMContext):
    ...
    for i in list_names:
        ...
        session.commit()
    logger.success(f'USER "{msg.from_user.id} - {msg.from_user.username}" ADD {len(list_add_names)} PART + NO ADD - {len(no_add_names)}')
...
async def add_participant_db(msg: types.Message, state: FSMContext):
    ...
    if check_name > 0:
        ...
        session.add(new_participant)
        session.commit()
        logger.success(f'USER "{msg.from_user.id} - {msg.from_user.username}" ADD PART')
...
async def choose_participant_for_del(call: types.CallbackQuery):
    ...
    await call.answer()
    logger.success(f'USER "{call.from_user.id} - {call.from_user.username}" PUSH DEL PART')
...
async def del_event(call: types.CallbackQuery, callback_data: dict):
    ...
    if len(session.query(Participant).filter(Participant.id == callback_data['participant_id']).first().expenses) > 0:
        ...
        logger.success(f'USER "{call.from_user.id} - {call.from_user.username}" NO DEL PART')
    else:
        ...
        session.commit()
        logger.success(f'USER "{call.from_user.id} - {call.from_user.username}" DEL PART')
